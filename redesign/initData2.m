...
qq=struct('circle', repmat({struct('dat',repmat({zeros(1079,1)}, [1,azimuthalSetSize]))} , [1,ntimesteps]));
...
qq=struct('circle', repmat({struct('dat',repmat({zeros(1,1080)}, [1,1080]))} , [1,ncs]));
elseif initStr=="avgPreFft_noCsYet" %redo
  qq=struct('dat',repmat({zeros(1080,1)}, [1,540]));
